...
public class DefaultSavingsPotentialService extends BaseService implements ISavingsPotentialService {
    ...
    public UUID create(AuthenticatedUser user, String name, TemporalSavingsConsumerSelectionFilter parameters) {
        ...
        try {
            ...
            request.param(
        		Constants.PARAMETER_NAME_DELIMITER + "execution.mode",
        		"SAVINGS"
    		);

            // Set scenario key
            request.param(
        		Constants.PARAMETER_NAME_DELIMITER + "scenario.key",
        		key.toString()
    		);          
           
            this.schedulerClient.getObject().launch(request, "SAVINGS-POTENTIAL");
        } catch(Exception ex) {
