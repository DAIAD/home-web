...
import eu.daiad.common.domain.application.AccountWhiteListEntity;
import eu.daiad.common.domain.application.DeviceAmphiroConfigurationEntity;
import eu.daiad.common.domain.application.DeviceAmphiroDefaultConfigurationEntity;
import eu.daiad.common.domain.application.DeviceAmphiroEntity;
import eu.daiad.common.domain.application.DeviceAmphiroPermissionEntity;
import eu.daiad.common.domain.application.DeviceEntity;
import eu.daiad.common.domain.application.DeviceMeterEntity;
import eu.daiad.common.domain.application.DevicePropertyEntity;
import eu.daiad.common.model.KeyValuePair;
import eu.daiad.common.model.device.AmphiroDevice;
import eu.daiad.common.model.device.AmphiroDeviceUpdate;
...
import eu.daiad.common.model.device.DeviceUpdate;
...
public class JpaDeviceRepository extends BaseRepository implements IDeviceRepository {
                    ...
                    case AMPHIRO:
                        if ((query.getType() == EnumDeviceType.UNDEFINED) || (query.getType() == entity.getType())) {
                            ...
                            for (DeviceAmphiroConfigurationEntity c : amphiroEntity.getConfigurations()) {
                                if (c.isActive()) {
                                    amphiro.setConfiguration(new eu.daiad.common.model.device.DeviceAmphiroConfiguration(c));
                                    break;
                                }
                            }
        ...
        try {
            for (UUID deviceKey : deviceKeys) {
                ...
                for (DeviceAmphiroConfigurationEntity c : device.getConfigurations()) {
                    if (c.isActive()) {
                        eu.daiad.common.model.device.DeviceAmphiroConfiguration configuration = new eu.daiad.common.model.device.DeviceAmphiroConfiguration(
                                        c);
    ...
    public void notifyConfiguration(UUID userKey, UUID deviceKey, UUID version, DateTime updatedOn)
        ...
        try {
            ...
            if (!found) {
                throw createApplicationException(DeviceErrorCode.CONFIGURATION_NOT_FOUND)
        ...
        for (DeviceUpdate update : request.getUpdates()) {
            switch (update.getType()) {
                case AMPHIRO:
                    ...
                    if (result.size() != 1) {
                        throw createApplicationException(DeviceErrorCode.NOT_FOUND)
                        	.set("key", update.getKey().toString());
                    } else {
                        // Set new values
                    ...
                    break;
                default:
                    throw createApplicationException(DeviceErrorCode.NOT_SUPPORTED)
                    	.set("type", update.getType().toString());
            }
        }
    }
    ...
    @Override
    public List<eu.daiad.common.model.device.DeviceAmphiroConfiguration> getDeviceConfigurationHistory(UUID deviceKey) {
        List<eu.daiad.common.model.device.DeviceAmphiroConfiguration> configurations = new ArrayList<eu.daiad.common.model.device.DeviceAmphiroConfiguration>();
        ...
        for (DeviceAmphiroConfigurationEntity c : device.getConfigurations()) {
            eu.daiad.common.model.device.DeviceAmphiroConfiguration configuration = new eu.daiad.common.model.device.DeviceAmphiroConfiguration(c);

            configurations.add(configuration);
        }

        Collections.sort(configurations, new Comparator<eu.daiad.common.model.device.DeviceAmphiroConfiguration>() {

            @Override
            public int compare(eu.daiad.common.model.device.DeviceAmphiroConfiguration c1,
                               eu.daiad.common.model.device.DeviceAmphiroConfiguration c2) {
