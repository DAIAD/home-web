...
import eu.daiad.common.domain.application.AccountProfileEntity;
import eu.daiad.common.domain.application.AccountProfileHistoryEntity;
...
import eu.daiad.common.model.device.DeviceRegistration;
...
import eu.daiad.common.model.profile.EnumUnit;
import eu.daiad.common.model.profile.EnumUtilityMode;
import eu.daiad.common.model.profile.EnumWebMode;
import eu.daiad.common.model.profile.Household;
import eu.daiad.common.model.profile.HouseholdMember;
...
import eu.daiad.common.model.profile.ProfileModeChange;
import eu.daiad.common.model.profile.ProfileModes;
import eu.daiad.common.model.profile.ProfileModesChanges;
...
public class JpaProfileRepository extends BaseRepository implements IProfileRepository {
    ...
    public Profile getProfileByUserKey(UUID userKey, EnumApplication application) throws ApplicationException
    {
        try {
            // Load account data
            TypedQuery<eu.daiad.common.domain.application.AccountEntity> userQuery = entityManager.createQuery(
            ...
            for (ProfileModeChange modeChange : modeChanges.getChanges()) {
                switch (modeChange.getMode()) {
                    case "amphiro":
                        ...
                        Set<eu.daiad.common.domain.application.DeviceEntity> devices = account.getDevices();
                        for (eu.daiad.common.domain.application.DeviceEntity device : devices) {
    ...
    public ProfileModesFilterOptions getFilterOptions() throws ApplicationException {
        ...
        for (AccountEntity a : userAccounts) {
            int amphiroCount = 0;
            for (eu.daiad.common.domain.application.DeviceEntity d : a.getDevices()) {
    ...
    public void saveProfile(UpdateProfileRequest updates) throws ApplicationException {
        ...
        if (!StringUtils.isBlank(updates.getConfiguration())) {
            switch (updates.getApplication()) {
                ...
                default:
                    throw createApplicationException(ProfileErrorCode.PROFILE_NOT_SUPPORTED)
                    	.set("application", updates.getApplication());
            }
        }
        ...
        Collections.sort(members, new Comparator<HouseholdMember>() {
            ...
            public int compare(HouseholdMember first, HouseholdMember second) {
                if (first.getIndex() == second.getIndex()) {
                    throw createApplicationException(ProfileErrorCode.HOUSEHOLD_MEMBER_DUPLICATE_INDEX)
                    	.set("index", first.getIndex());
