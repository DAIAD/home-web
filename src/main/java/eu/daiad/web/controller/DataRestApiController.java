...
public class DataRestApiController {
	...
	private static final int ERROR_PERMISSION_DENIED = 5;
	...
	public RestResponse insert(@RequestBody MeasurementCollection data,
		...
		try {
			...
			ApplicationUser user = this.authenticator.authenticateAndGetUser(data
					.getCredentials());
			if (user == null) {
			...
			case AMPHIRO:
				if (data instanceof DeviceMeasurementCollection) {
					...
					Device device = this.deviceRepository.getUserDeviceByKey(
							deviceData.getDeviceKey(), data.getUserKey());
					if (device == null) {
					...
					measurementRepository.storeDataAmphiro((ApplicationUser) user,
							(AmphiroDevice) device,
			...
			case METER:
				if (data instanceof MeterMeasurementCollection) {
					measurementRepository
							.storeDataSmartMeter((MeterMeasurementCollection) data);
				}
				break;
			default:
				break;
	...
	public ResponseEntity<InputStreamResource> download(
			@RequestBody DownloadFileRequest data, BindingResult results)
		...
		try {
			...
			logger.error(
					String.format("File [%s] was not found.", data.getToken()),
					ex);
		}
