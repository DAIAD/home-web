...
public class UserRepository {
	...
	@Autowired
	public UserRepository(@Value("${hbase.zookeeper.quorum}") String quorum) {
		this.quorum = quorum;
	}
	...
	public void createDefaultUser() {
		...
		try {
			...
			p.addColumn(columnFamily, column, StringUtils.join(defaultAdminRoles, ",")
					.getBytes(StandardCharsets.UTF_8));
	...
	public ApplicationUser createUser(String username, String password,
		...
		try {
			...
			column = Bytes.toBytes("roles");
			p.addColumn(columnFamily, column,
					EnumRole.ROLE_USER.toString().getBytes(StandardCharsets.UTF_8));
	...
	public void setRole(String username, EnumRole role, boolean set) throws Exception {
		...
		try {
			...
			p.addColumn(columnFamily, column,
					StringUtils.join(roles,",").getBytes(StandardCharsets.UTF_8));
