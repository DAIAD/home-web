...
public class JpaDeviceRepository implements IDeviceRepository {
	...
	private static final Log logger = LogFactory
			.getLog(JpaDeviceRepository.class);
	...
	public UUID createAmphiroDevice(UUID userKey, String name,
		...
		try {
			...
			for (KeyValuePair p : properties) {
				amphiro.getProperties().add(
						new DeviceProperty(p.getKey(), p.getValue()));
			}
		...
		try {
			TypedQuery<eu.daiad.web.domain.Device> query = entityManager
					.createQuery(
							...
							eu.daiad.web.domain.Device.class).setFirstResult(0)
					.setMaxResults(1);
			...
			if (result.size() == 1) {
				...
				case AMPHIRO:
					...
					AmphiroDevice amphiro = new AmphiroDevice(
							amphiroEntiry.getKey(), amphiroEntiry.getName(),
							amphiroEntiry.getMacAddress());

					for (eu.daiad.web.domain.DeviceProperty p : amphiroEntiry
							.getProperties()) {
						amphiro.getProperties().add(
								new KeyValuePair(p.getKey(), p.getValue()));
					}
				...
				case METER:
					...
					WaterMeterDevice meter = new WaterMeterDevice(
							meterEntiry.getKey(), meterEntiry.getSerial());

					for (eu.daiad.web.domain.DeviceProperty p : meterEntiry
							.getProperties()) {
						meter.getProperties().add(
								new KeyValuePair(p.getKey(), p.getValue()));
					}
	...
	public ArrayList<Device> getUserDevices(UUID userKey,
		...
		try {
			TypedQuery<eu.daiad.web.domain.Device> typedQuery = entityManager
					.createQuery(
							"select d from device d where d.account.key = :user_key",
							eu.daiad.web.domain.Device.class).setFirstResult(0);
			typedQuery.setParameter("user_key", userKey);

			List<eu.daiad.web.domain.Device> result = typedQuery
					.getResultList();
			...
			for (eu.daiad.web.domain.Device entity : result) {
				...
				case AMPHIRO:
					if ((query.getType() == EnumDeviceType.UNDEFINED)
							|| (query.getType() == entity.getType())) {
						eu.daiad.web.domain.DeviceAmphiro amphiroEntiry = (eu.daiad.web.domain.DeviceAmphiro) entity;

						AmphiroDevice amphiro = new AmphiroDevice(
								amphiroEntiry.getKey(),
								amphiroEntiry.getName(),
								amphiroEntiry.getMacAddress());
	...
	public Device getUserAmphiroDeviceByMacAddress(UUID userKey,
		...
		try {
			TypedQuery<eu.daiad.web.domain.DeviceAmphiro> query = entityManager
					.createQuery(
							"select d from device_amphiro d where d.macAddress = :macAddress",
							eu.daiad.web.domain.DeviceAmphiro.class)
					.setFirstResult(0).setMaxResults(1);
			query.setParameter("macAddress", macAddress);

			List<eu.daiad.web.domain.DeviceAmphiro> result = query
					.getResultList();

			if (result.size() == 1) {
				eu.daiad.web.domain.DeviceAmphiro entity = result.get(0);

				AmphiroDevice amphiro = new AmphiroDevice(entity.getKey(),
						entity.getName(), entity.getMacAddress());

				for (eu.daiad.web.domain.DeviceProperty p : entity
						.getProperties()) {
					amphiro.getProperties().add(
							new KeyValuePair(p.getKey(), p.getValue()));
				}
		...
		try {
			TypedQuery<eu.daiad.web.domain.DeviceMeter> query = entityManager
					.createQuery(
							...
							eu.daiad.web.domain.DeviceMeter.class)
					.setFirstResult(0).setMaxResults(1);
			query.setParameter("serial", serial);

			List<eu.daiad.web.domain.DeviceMeter> result = query
					.getResultList();

			if (result.size() == 1) {
				eu.daiad.web.domain.DeviceMeter entity = result.get(0);

				WaterMeterDevice meter = new WaterMeterDevice(entity.getKey(),
						entity.getSerial());
