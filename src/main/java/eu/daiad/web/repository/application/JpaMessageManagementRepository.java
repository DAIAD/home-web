...
public class JpaMessageManagementRepository extends BaseRepository implements IMessageManagementRepository {
	...
	private Alert getAlertByType(EnumAlertType type) {
		TypedQuery<eu.daiad.web.domain.application.Alert> query = entityManager
						.createQuery("select a from alert a where a.id = :id",
										eu.daiad.web.domain.application.Alert.class).setFirstResult(0).setMaxResults(1);
	...
	private DynamicRecommendation getDynamicRecommendationByType(EnumDynamicRecommendationType type) {
		TypedQuery<eu.daiad.web.domain.application.DynamicRecommendation> query = entityManager
						.createQuery("select d from dynamic_recommendation d where d.id = :id",
										eu.daiad.web.domain.application.DynamicRecommendation.class).setFirstResult(0)
						.setMaxResults(1);
	...
	private void recommendLowerTemperatureAmphiro(MessageCalculationConfiguration config,
		...
		if (DateTime.now().getDayOfMonth() == config.getComputeThisDayOfMonth()) {
			if (status.getRecommendLowerTemperatureAmphiro().getKey()) {
				...
				setAccountDynamicRecommendationProperty(recommendation, config.getCurrencyKey1(),
								eurosSavedPerYear.toString());
				setAccountDynamicRecommendationProperty(recommendation, config.getCurrencyKey2(),
								eurosSavedPerYear.toString());
			}
		}
	}
	...
	private void recommendLowerFlowAmphiro(MessageCalculationConfiguration config,
		...
		if (DateTime.now().getDayOfMonth() == config.getComputeThisDayOfMonth()) {
			if (status.getRecommendLowerFlowAmphiro().getKey()) {
				AccountDynamicRecommendation recommendation = createAccountDynamicRecommendation(account,
								getDynamicRecommendationByType(EnumDynamicRecommendationType.LOWER_FLOW),
								DateTime.now());
	...
	private void recommendShowerHeadChangeAmphiro(MessageCalculationConfiguration config,
		...
		if (DateTime.now().getDayOfMonth() == config.getComputeThisDayOfMonth()) {
			if (status.getRecommendShowerHeadChangeAmphiro().getKey()) {
				...
				setAccountDynamicRecommendationProperty(recommendation, config.getIntKey1(),
								annualLitresSaved.toString());
				setAccountDynamicRecommendationProperty(recommendation, config.getIntKey2(),
								annualLitresSaved.toString());
			}
		}
	}
	...
	private void recommendShampooAmphiro(MessageCalculationConfiguration config,
		...
		if (DateTime.now().getDayOfMonth() == config.getComputeThisDayOfMonth()) {
			if (status.getRecommendShampooChangeAmphiro().getKey()) {
				AccountDynamicRecommendation recommendation = createAccountDynamicRecommendation(account,
								getDynamicRecommendationByType(EnumDynamicRecommendationType.SHAMPOO_CHANGE),
								DateTime.now());
	...
	private void recommendReduceFlowWhenNotNeededAmphiro(MessageCalculationConfiguration config,
		...
		if (DateTime.now().getDayOfMonth() == config.getComputeThisDayOfMonth()) {
			if (status.getRecommendReduceFlowWhenNotNeededAmphiro().getKey()) {
				...
				setAccountDynamicRecommendationProperty(recommendation, config.getIntKey1(),
								moreShowerWaterThanOthers.toString());
				setAccountDynamicRecommendationProperty(recommendation, config.getIntKey2(),
								moreShowerWaterThanOthers.toString());
			}
		}
	...
	private boolean isAlertAlreadyProducedForUser(int alertId, Account account) {
		TypedQuery<eu.daiad.web.domain.application.AccountAlert> query = entityManager
						.createQuery("select a from account_alert a where a.account.id = :accountId and a.alert.id = :alertId",
