...
lambda_cv = function(x,y,k=5){
  len       = length(y)
  fold_size =  floor(len/k)
  ers = c()
  lam = glmnet(x,y,nlambda=20,alpha=0.5)$lambda
  for(i in 0:(k-1)){
    ...
    test_x   = x[test_ind,]
    test_y   = y[test_ind]
    mdl  = glmnet(train_x,train_y,lambda=lam,alpha=0.5)
    pred = predict(mdl,test_x,nlambda=lam)
    buf  = sqrt(colSums((pred-test_y)**2)/fold_size)
    if(length(buf)==length(lam))
      ers = c(ers,buf)
  }
...
train_test_24 = function(d,start,end,test_points,perm=FALSE){
  ...
  mdl  = glmnet(x[start:(test_start-1),],y[start:(test_start-1)],lambda =c(lam),alpha=0.5)
...
get_train_res = function(user,h2,t0){
  ...
  x = d[,1:31]
  y = d[,32] 
  lam0 = lambda_cv(x[1:t0,],y[1:t0])
 # lam0 = 0
  mdl0 = glmnet(x[1:t0,],y[1:t0],lambda=c(lam0),alpha=0.5)
...
compare_methods = function(user,h2,t0,min_size,test_size,ahead,window,thresh){
  ...
  if(sel[[1]]==1)
    ...
    lam1  = lambda_cv(x[sel[[1]]:t0,],y[sel[[1]]:t0])
  mdl1  = glmnet(x[sel[[1]]:t0,],y[sel[[1]]:t0],lambda=c(lam1),alpha=0.5)
  ...
  if(sel[[2]]==1)
    ...
    lam2 = lambda_cv(x[sel[[2]]:t0,],y[sel[[2]]:t0])
  mdl2 = glmnet(x[sel[[2]]:t0,],y[sel[[2]]:t0],lambda=c(lam2),alpha=0.5)
  ...
  if(sel[[3]]==1)
    ...
    lam3 = lambda_cv(x[sel[[3]]:t0,],y[sel[[3]]:t0])
  mdl3 = glmnet(x[sel[[3]]:t0,],y[sel[[3]]:t0],lambda=c(lam3),alpha=0.5)
  ...
  for(u in start:end){
    if(monthly_filter(users[[u]]))
      for( t in c(80,120,160,190,200,230,240,230,270,280,310,320,340)){
        ...
        print(paste(u,t,sep=" "))
        res = tryCatch(compare_methods(users[[u]],7,t,min_size,test_size,ahead,window,thresh),error=function(err){
          print(err)
          return(-1)
        })
...
parse_results = function(res,t0){
  ...
  r = c(r,mean((ers[1,]-ers[2,])/norm))
  r = c(r,mean((ers[1,]-ers[3,])/norm))
  r = c(r,mean((ers[1,]-ers[4,])/norm))
  r = c(r,mean((ers[1,]-ers[5,])/norm))
...
remove_period = function(user,h2,t0,t1,t2,ahead=10){
  ...
  lam0 = lambda_cv(x,y)
  mdl0 = glmnet(x,y,lambda=c(lam0),alpha=0.5)
  pred0 = predict(mdl0,dt[(t0+1):(t0+ahead),1:31])
  er0 = mean(abs(dt[(t0+1):(t0+ahead),32] - pred0))
...
gradients = function(user,h2,t1,t0){
  ...
  if(any(y>10000)||length(y)>10000)
    return(-1)
  lam0 = lambda_cv(x[t1:t0,],y[t1:t0])
  mdl0 = glmnet(x[t1:t0,],y[t1:t0],lambda=c(lam0),alpha=0.5)
