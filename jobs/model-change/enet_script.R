...
select_start = function(d,t0,min_size=40,test_size=15,window=20,thresh=0.05,type="stat"){
  ...
  plot(ers[[2]],rowSums(abs(ers[[1]]))/test_size)
...
compare_methods = function(d,t0,min_size,test_size,ahead,window,thresh,type){
...
test=function(data,min_train_size=10,val_size=25,points_ahead=10,window=10,thresh=0.2,type='stat'){
  ers = c()
  times = c()
