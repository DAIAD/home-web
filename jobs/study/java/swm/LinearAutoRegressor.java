...
public class LinearAutoRegressor implements Regressor{
	...
	public void train(AllData allData,double ratio,int k,int b){
		...
		X=new SimpleMatrix(allData.data.size()-b,allData.metadata.get(0).values.length*b+1);
		Y=new SimpleMatrix(allData.data.size()-b,1);
		
		int trainEnd = (int)(ratio*(allData.data.size()-b));
		
		for(int i=b;i<allData.data.size();i++){
			for(int l=0;l<allData.metadata.get(0).values.length;l++)
				for(int j=0;j<b;j++)
					X.set(i-b,l*b+j,allData.metadata.get(i-j-1).values[l]);
	
			X.set(i-b,allData.metadata.get(0).values.length*b,1);
